...
export default async function FightersPage({ params }: Props) {
    ...
    <main className="min-h-screen bg-black text-white">
      <header className="border-b border-neutral-800">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">{leagueConfig.name} Fighters</h1>
              <p className="text-neutral-500 text-sm">{athletes.length} fighters</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="tab-nav mb-6">
          ...
          <Link href={`/sport/${sport}/${league}/fighters`} className="tab-item active">
            Fighters
          </Link>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {athletes.map((athlete: any) => (
            <Link
              key={athlete.id}
              href={`/sport/${sport}/${league}/fighters/${athlete.id}`}
            ...
            >
              {athlete.headshot?.href ? (
                <img
                  ...
                  className="w-20 h-20 object-cover mx-auto mb-3 rounded-full border-2 border-neutral-700 group-hover:border-red-600 transition-colors"
                />
              ) : (
                <div className="w-20 h-20 bg-neutral-800 rounded-full mx-auto mb-3 flex items-center justify-center text-xl font-bold text-neutral-500 border-2 border-neutral-700 group-hover:border-red-600 transition-colors">
              ...
              {athlete.position?.name && (
                <p className="text-neutral-500 text-xs mt-1">{athlete.position.name}</p>
              )}
              {athlete.flag?.href && (
              ...
              {athlete.record && (
                <p className="text-neutral-400 text-xs mt-1 font-medium">{athlete.record}</p>
              )}
            </Link>
          ))}
        </div>
        ...
        {athletes.length === 0 && (
          <div className="text-center py-16 text-neutral-500">
            <p className="text-lg">No fighters found</p>
            <p className="text-sm mt-2">Fighter data may not be available for this league</p>
          </div>
        )}
      </div>
    </main>
